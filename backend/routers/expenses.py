...
def delete_expense(
    ...
    if not expense:
        raise HTTPException(status_code=404, detail="Expense not found")
    if db.query(models.PayrollRun).filter(models.PayrollRun.expense_id == expense.id).first():
        raise HTTPException(
            status_code=400,
            detail="This expense was generated by a payroll run — void that payroll run instead",
        )
    audit.log(
